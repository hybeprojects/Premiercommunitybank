          ...
          <div className="space-y-2">
            {Array.isArray(txs) && txs.length ? txs.map((t: any) => (
              <div key={t.id} className="flex items-center justify-between border rounded-md p-2">
                <div>
                  <div className="text-sm text-gray-200 md:text-gray-700">{t.description || 'Transfer'}</div>
                  <div className="text-xs text-gray-400 md:text-gray-500">{t.direction} • {t.status}</div>
                </div>
